...
UrbanSound inputs are pre-saved tensors (loaded directly with torch.load),
so there is no resampling step -- the signal is used at its stored length.
"""
...
def main():
    ...
    data_params = {"signal_length": signal_length}
    ...
    for epoch in range(config["epochs"]):
        ...
        for i, (signals, labels) in enumerate(train_loader):
            ...
            signals = signals.reshape(batch_size, 1, signal_length)
