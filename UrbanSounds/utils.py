...
class UrbanSound8k(Dataset):
    def __init__(self, csv_path):
        ...
        # Tensor paths in the CSV's file_dir column are resolved relative to
    ...
    def __getitem__(self, index):
        ...
        # resolve the tensor path relative to audio_root so loading does not
        ...
        if not audio_path.exists():
            raise FileNotFoundError(
                ...
                "  CSV path    : {}\n"
                "  audio_root  : {}\n"
                "Paths are resolved as <two dirs above the CSV> / file_dir. "
        ...
        # UrbanSound inputs are pre-saved tensors, loaded directly
...
def get_data(data_dir, bs_train, bs_test, device, data_params):
    ...
    train_set = UrbanSound8k(data_dir + "train.csv")
    test_set = UrbanSound8k(data_dir + "test.csv")
    ...
    def pad_sequence(batch):
        ...
        batch.append(torch.zeros(signal_length,1, device=device))
...
def evaluation(data_loader, model, batch_size, objective, device, data_params):
    model.eval()
    correct = 0
    test_loss = 0
    with torch.no_grad():
        for i, (signals, labels) in enumerate(data_loader):
            ...
            # no resampling: pre-saved tensors are used as-is
            ...
            signals = signals.reshape(batch_size, 1, data_params["signal_length"])
